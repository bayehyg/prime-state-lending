  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-xs font-medium text-body mb-2 uppercase tracking-wide">Annual Income</label>
      ...
      <div className="bg-white border border-edge rounded-2xl p-8">
        <div className="text-center mb-6">
          <span className="block text-sm text-body mb-2">Maximum Home Price You Can Afford</span>
        ...
        <div className="grid grid-cols-2 gap-4 pt-6 border-t border-edge/50">
          <div>
            <span className="block text-xs text-body mb-1">Max Monthly Payment</span>
            <span className="text-xl font-semibold text-heading">${maxMonthlyPayment.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
          <div>
            <span className="block text-xs text-body mb-1">Max Loan Amount</span>
            <span className="text-xl font-semibold text-heading">${maxLoanAmount.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
          </div>
        </div>

        <div className="mt-6 p-4 bg-accent/10 border border-accent/20 rounded-lg">
          <p className="text-xs text-accent-hover">Based on 43% debt-to-income ratio. Actual approval may vary based on credit score and other factors.</p>
        </div>
      </div>
    </div>
  );
